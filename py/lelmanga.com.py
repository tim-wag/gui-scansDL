...
async def get_chap_image_link(soup):
    return soup.find_all('img', class_="attachment- size- wp-post-image")[0]['src']

async def get_chap_links(soup) :
    links = {} # {Chapter/Volume name : link}

    for li in [a for a in soup.find("div", id="chapterlist").findChildren(name='ul')[0].contents if a!='\n']:
...
async def main () :
    ...
    if command == 'pageExists' :  
        input = args[1]
        link = f'https://lelmanga.com/manga/{await normalizeInput(input)}'
